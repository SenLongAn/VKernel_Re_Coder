...
namespace VKernel
{
    ...
    void VulkanAPI::prepareContext() { m_current_command_buffer = m_command_buffers[m_current_frame_index]; }
    ...
    bool VulkanAPI::prepareBeforePass(std::function<void()> passUpdateAfterRecreateSwapchain)
    {
        ...
        VkResult res_begin_command_buffer =
            vkBeginCommandBuffer(m_command_buffers[m_current_frame_index], &command_buffer_begin_info);
    ...
    void VulkanAPI::submitRendering(std::function<void()> passUpdateAfterRecreateSwapchain)
    {
        ...
        submit_info.pWaitSemaphores =
            &m_image_available_for_render_semaphores[m_current_frame_index]; ///< Wait Semaphores
        ...
        VkResult res_queue_submit = vkQueueSubmit(
            m_graphics_queue, 1, &submit_info, m_is_frame_in_flight_fences[m_current_frame_index]); ///< Queue Submit
    ...
    void VulkanAPI::createBuffer(VkDeviceSize          size,
    ...
    void VulkanAPI::copyBuffer(VkBuffer     srcBuffer,
                               VkBuffer     dstBuffer,
    ...
    void VulkanAPI::createImage(uint32_t              image_width,
    ...
    void VulkanAPI::createImageView(VkImage            image,
                                    VkFormat           format,
                                    ...
                                    VkImageView&       image_view)
    {
        image_view = VulkanUtil::createImageView(
    ...
    VkDevice VulkanAPI::getLogicDevice() const { return m_device; }
    ...
    uint8_t VulkanAPI::getMaxFramesInFlight() const { return k_max_frames_in_flight; }

    VkDescriptorPool VulkanAPI::getDescriptorPool() const { return m_descriptor_pool; }

    VkCommandBuffer VulkanAPI::getCurrentCommandBuffer() const { return m_current_command_buffer; }

    const VkFence* VulkanAPI::getFenceList() const { return m_is_frame_in_flight_fences; }

    uint8_t VulkanAPI::getCurrentFrameIndex() const { return m_current_frame_index; }

    VkCommandPool VulkanAPI::getCommandPool() const { return m_command_pool; }

    uint8_t VulkanAPI::getCurrentSwapchainImageIndex() const { return m_current_swapchain_image_index; }
    ...
    VkPhysicalDevice VulkanAPI::getPhysicalDevice() const { return m_physical_device; }
    ...
    QueueFamilyIndices VulkanAPI::findQueueFamilies(VkPhysicalDevice physicalm_device)
    {
        ...
        for (const auto& queue_family : queue_families)
        {
            ...
            vkGetPhysicalDeviceSurfaceSupportKHR(physicalm_device, i, m_surface, &is_present_support);
    ...
    void VulkanAPI::destroyImageView(VkImageView imageView) { vkDestroyImageView(m_device, imageView, nullptr); }
    ...
    void VulkanAPI::createCommandPool()
    {
        // create command pools
        {
            ...
            command_pool_create_info.flags =
                VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT; ///< Allow resetting a single command buffer
        ...
        // create command pools
        {
            ...
            command_pool_create_info.pNext = NULL;
            command_pool_create_info.flags =
                VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT; ///< The command buffer will have a short lifespan.
            command_pool_create_info.queueFamilyIndex = m_queue_indices.graphics_family.value();
