...
namespace VKernel
{
    ...
    void FXAAPass::setupPipelines()
    {
        ...
        // Shader Module
        VkShaderModule vert_shader_module = m_vulkan_api->createShaderModule(FXAA_VERT);
        VkShaderModule frag_shader_module = m_vulkan_api->createShaderModule(FXAA_FRAG);
    ...
    void FXAAPass::updateAfterFramebufferRecreate(VkImageView input_attachment)
    {
        ...
        post_process_per_frame_input_attachment_info.sampler =
            m_vulkan_api->getOrCreateDefaultSampler(Default_Sampler_Linear);
    ...
    void FXAAPass::draw()
    {
        ...
        VkViewport    viewport        = {0.0,
                                         0.0,
                                         static_cast<float>(swap_chain_desc.extent.width),
                                         static_cast<float>(swap_chain_desc.extent.height),
                                         0.0,
                                         1.0};
        int32_t       x               = static_cast<int32_t>(swap_chain_desc.viewport.x);
        int32_t       y               = static_cast<int32_t>(swap_chain_desc.viewport.y);
        uint32_t      width           = static_cast<uint32_t>(swap_chain_desc.viewport.width);
        uint32_t      height          = static_cast<uint32_t>(swap_chain_desc.viewport.height);
