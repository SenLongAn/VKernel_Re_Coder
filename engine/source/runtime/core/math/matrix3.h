...
namespace VKernel
{
    ...
    public:
        ...
        void fromData(float (&float_array)[9]) ///< input data
        {
            m_mat[0][0] = float_array[0];
            m_mat[0][1] = float_array[1];
            m_mat[0][2] = float_array[2];
        ...
        void toData(float (&float_array)[9]) const ///< output data
        {
        ...
        void setColumn(size_t iCol, const Vector3& vec); ///< set column
        void fromAxes(const Vector3& x_axis, const Vector3& y_axis, const Vector3& z_axis); ///< set column
        ...
        // util function
        //-----------------------------------------------------------------------
        // Retrieve columns through the index
        ...
        // A^T, Read horizontally and write vertically.
        ...
        // QDU decomposition
        // Q: Orthogonal matrix (rotation) 
        // D: Diagonal Matrix (Scaling) 
        // U: Upper triangular matrix (slicing)
        void calculateQDUDecomposition(Matrix3x3& out_Q, Vector3& out_D, Vector3& out_U) const;

        // Convert to axis based on angle, matrix must be orthonormal
