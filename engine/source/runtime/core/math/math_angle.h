...
/**
 * basic math and angle
 */

#define CMP(x, y) (fabsf(x - y) < FLT_EPSILON * fmaxf(1.0f, fmaxf(fabsf(x), fabsf(y)))) ///< Accurately compare whether two floating-point numbers are approximately equal
...
namespace VKernel
{
    ...
    // log
    static const float Math_LOG2         = log(2.0f); ///< The logarithm of 2 to the base e, The value is approximately 0.69314718056
    
    // Approaching zero
    static const float Math_EPSILON      = 1e-6f; ///< Floating-point number error, The value is approximately 0.000001
    ...
    // Angle: Wrapper class which identifies a value as the currently default angle type
    //----------------------------------------------------------------------- 
    class Angle
    {
        float m_angle;
    ...
    public:
        ...
        // The user-defined type conversion operator explicitly converts the Angle class to the Radian class.
        // Angle angle(45.0f); Radian rad = static_cast<Radian>(angle);
    ...
    // Degree needs to know the complete definition of Radian, and Radian needs to know the complete definition of Degree
    // solution:
    // Degree forward declaration + Radian class type definition + Degree class type definition + Radian class external member definition
