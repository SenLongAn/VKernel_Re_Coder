...
/**
 * Metadata information: annotation declaration, registration interface, access information interface
 */

namespace VKernel
{

// when building the parser, Add annotation
#if defined(__REFLECTION_PARSER__) ///< In arguments defined in parser.h
#define META(...) __attribute__((annotate(#__VA_ARGS__)))
#define CLASS(class_name, ...) class __attribute__((annotate(#__VA_ARGS__))) class_name
#define STRUCT(struct_name, ...) struct __attribute__((annotate(#__VA_ARGS__))) struct_name
#else // As a regular class during runtime construction
#define META(...)
...
// ReflectionInstance: Create non-pointers and pointers
#define TypeMetaDef(class_name, ptr) \
    VKernel::Reflection::ReflectionInstance(VKernel::Reflection::TypeMeta::newMetaFromName(#class_name), \
                                            (class_name*)ptr)

#define TypeMetaDefPtr(class_name, ptr) \
    new VKernel::Reflection::ReflectionInstance(VKernel::Reflection::TypeMeta::newMetaFromName(#class_name), \
                                                (class_name*)ptr)
    ...
    namespace Reflection
    {
        ...
        public:
            ...
            // Serialization and Deserialization，It will call _generated/serializer， Function for Specialized Template
            // Class Type
            ...
            std::string getTypeName(); ///< 类类型名

            int getFieldsList(FieldAccessor*& out_list);   ///< field list
            int getMethodsList(MethodAccessor*& out_list); ///< method list

            int getBaseClassReflectionInstanceList(ReflectionInstance*& out_list,
                                                   void* instance); ///< baise class ReflectionInstance list

            FieldAccessor  getFieldByName(const char* name);  ///< FieldAccessor
            MethodAccessor getMethodByName(const char* name); ///< MethodAccessor

            bool isValid() { return m_is_valid; } ///< is valid

            // Copy assignment function
            TypeMeta& operator=(const TypeMeta& dest);

        private:
            TypeMeta(std::string type_name); ///< private construct
        ...
        // Field Metadata information interface
        class FieldAccessor
        {
        ...
        public:
            ...
            bool getTypeMeta(TypeMeta& field_type); ///< true: it's a reflection type, false: it's not a reflection type
        ...
        // method Metadata information interface
        class MethodAccessor
        {
            friend class TypeMeta;
        ...
        // array Metadata information interface
        class ArrayAccessor
        {
            friend class TypeMeta;
