...
{
    List children; ///< std::vector<Cursor>

    // lambda : visitor is used to determine whether to continue searching
    auto visitor = [](CXCursor cursor, CXCursor parent, CXClientData data) {
        auto container = static_cast<List*>(data);

        container->emplace_back(cursor);

        if (cursor.kind == CXCursor_LastPreprocessing) ///< Need to stop traversing
            return CXChildVisit_Break;

        return CXChildVisit_Continue; ///< continue
    };

    clang_visitChildren(m_handle, visitor, &children); ///< Traverse all direct nodes
