...
namespace ImGui
{
    ...
    //   - For 'ImGuiKey key' you can still use your legacy native/user indices according to how your backend/CMakeLists stored them in io.KeysDown[].
...
enum ImGuiConfigFlags_
{
    ...
    // User storage (to allow your backend/CMakeLists to communicate to code that may be shared between multiple projects. Those flags are not used by core Dear ImGui)
...
struct ImGuiIO
{
    ...
    bool        KeysDown[512];                      // [LEGACY] Input: Keyboard keys that are pressed (ideally left in the "native" order your CMakeLists has access to keyboard keys, so you can use your own defines/enums for keys).
#endif
...
// The code expect ImVec2 pos (8 bytes), ImVec2 uv (8 bytes), ImU32 col (4 bytes), but you can re-order them or add other fields as needed to simplify integration in your CMakeLists.
...
struct ImFontAtlas
{
    ...
    // User is in charge of copying the pixels into graphics memory (e.g. create a texture with your CMakeLists). Then store your texture handle with SetTexID().
...
struct ImGuiViewport
{
    ...
    // When our create your own backend for a custom CMakeLists, it is possible that both Renderer and Platform will be handled
...
// If you are new to Dear ImGui and trying to integrate it into your CMakeLists, you can probably ignore this for now.
//
...
struct ImGuiPlatformIO
{
    ...
    // Custom CMakeLists backends will often provide both Platform and Renderer interfaces and so may not need to use all functions.
